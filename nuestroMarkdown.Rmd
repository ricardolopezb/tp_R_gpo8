...
El principal proposito de este trabajo es el de lograr predecir con el conjunto de datos dado, con el mayor grado de certeza posible, que procedimiento se llevara a cabo en un paciente, de acuerdo a las siguientes variables: su edad, si es diabetico, su motivo de ingreso y el numero de lesiones. Elegimos estas variables porque...

Para hacerlo, se decidio usar el metodo RandomForest. Este modelo de prediccion genera...

With this model we obtained the following confusion matrix:

![](cuadro.png)

Con este modelo llegamos a predecir a donde iban dirigidos 94 de 104 pacientes utilizados para el testeo. Se obtuvo una precisión de aprxomimadamente un 90%. Respecto a este test, 10 pacientes que fueron a cirugía fueron predecidos para ir a angioplastía. El resultado de este modelo se obtuvo luego de probar diferentes cosas como aumentar el numero de arboles y agregar mas variables para hacer la prediccion.Se decidio mantener este predictor porque fue el de mejor accuracy que otuvimos, y tiene una baja cantidad de predicciones fallidas, las cuales se encuentran todas en predecir una angioplastia que era sirugia, lo cual segun lo investigado se trata del caso menos grave.
...
El valor de esta curva es de 0.861 (area bajo la curva). Cuanto mas cercano a uno es, se puede decir que mejor es el modelo. Con la cantidad de datos que se pasaron como input para el modelo y el dataset utilizado para trabajar, concluimos que es una buena curva.
