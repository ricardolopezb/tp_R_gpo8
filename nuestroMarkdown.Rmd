...
plotProcedimiento <-ggplot(counts, aes(fill=counts$ageInterval, y=counts$Freq, x=counts$Motivo)) + 
...
plotProcedimiento
```
Explanation: Corregir gráfico

## Proceidure distribution
...
```
Explanation: Corregir label y colores
...
```
Explanation: Corregir labely colores

## Number of injuries according to ACC
```{r, include = FALSE, result = 'asis'}
...
```
Explanation: Tocar colores iwi owo uwu

## Proceidure Distribution According the vasos afectados
...
## Relatin between TAD, TAS and clampeou time
```{r, include = FALSE, result = 'asis'}
...
abline(modeltad)
```
Explanation: mejorar 100% este gráfico

### TAS and clampeou time
