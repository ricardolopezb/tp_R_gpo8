...
The aim of this graph is to show the main causes of admission in the Hospital, according to the age of the patients. It is possible to easily see that Coronary Programmed Operations stands out as the most common reason.
...
```
 ...
 In this frequency graph it is possible to clearly see how CRM and RVAO are the most common types of surgery.
...
```
```{r fig.width=6, fig.height=6, fig.align='center',echo=FALSE,warning=FALSE}
barplot(t(as.matrix(procedACC_data)),beside=TRUE,main="Injuries according to ACC",xlab="Injuries", ylab="Frequency", col=c("#cd5c5c", "#6495ed"),
...
procedRisksData
```
```{r fig.width=7, fig.height=6, fig.align='center',echo=FALSE,warning=FALSE}
barplot(t(as.matrix(procedRisksData)),beside=TRUE,main="Procedure Frequency by Risk Factor",xlab="Risk Factors", ylab="Frequency", col=c("#cd5c5c", "#6495ed"),
